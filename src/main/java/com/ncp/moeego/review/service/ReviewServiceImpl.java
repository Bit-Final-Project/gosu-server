...
public class ReviewServiceImpl implements ReviewService {
    ...
    public Map<Long, List<String>> getImageUuidMapByReviewNos(List<Long> reviewNos) {
        List<Image> images = imageRepository.findImagesByReviewNos(reviewNos);

        return images.stream()
                .collect(Collectors.groupingBy(
                        image -> image.getReview().getReviewNo(),
                        Collectors.mapping(Image::getImageUuidName, Collectors.toList())
                ));
    }
    ...
    public Page<ItemReviewResponse> getReviewsByItemNo(Long proItemNo, int pg) {
        ...
        List<Long> reviewNos = reviewPage.getContent()
                .stream()
                .map(ItemReviewResponse::getReviewNo)
                .toList();

        Map<Long, List<String>> imageMap = getImageUuidMapByReviewNos(reviewNos);
